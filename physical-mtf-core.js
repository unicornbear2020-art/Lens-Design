...
const calculateLensComplexPupilMtf = (payload = {}) => {
  ...
  return {
    ...
    psf: payload.includeArrays === true ? { gridSize, values: normalizedPsf, energy: 1 } : null,
    otf: payload.includeArrays === true ? otf : null,
    referenceParity,
    elapsedMs: performance.now() - started,
    validation: {
...
const buildCanonicalAberrationPupil = ({ defocusWaves = 0, astigmatismWaves = 0 } = {}) => {
  const gridSize = 128;
  ...
  for (let y = 0; y < gridSize; y += 1) {
    for (let x = 0; x < gridSize; x += 1) {
      ...
      const wavefrontWaves = defocusWaves * (2 * radiusSquared - 1)
        + astigmatismWaves * (pupilX ** 2 - pupilY ** 2);
      const phase = 2 * Math.PI * wavefrontWaves;
      const index = y * gridSize + x;
      real[index] = Math.cos(phase);
      imag[index] = Math.sin(phase);
    }
  }
  return { gridSize, radiusPx, real, imag };
};

const canonicalMtfValueAt = (result, axis, normalizedFrequency) => {
  const curve = result?.mtf?.[axis] || [];
  if (!curve.length) return NaN;
  const target = clamp(normalizedFrequency, 0, 1);
  if (target <= curve[0].normalizedFrequency) return curve[0].value;
  for (let index = 1; index < curve.length; index += 1) {
    const right = curve[index];
    const left = curve[index - 1];
    if (target <= right.normalizedFrequency) {
      const span = right.normalizedFrequency - left.normalizedFrequency || 1;
      const fraction = (target - left.normalizedFrequency) / span;
...
const calculateCanonicalAberrationValidation = () => {
  if (canonicalAberrationValidationCache) return canonicalAberrationValidationCache;
  const definitions = [
    { key: "ideal", defocusWaves: 0, astigmatismWaves: 0 },
    { key: "positiveDefocus", defocusWaves: 0.25, astigmatismWaves: 0 },
    { key: "negativeDefocus", defocusWaves: -0.25, astigmatismWaves: 0 },
    { key: "astigmaticFocusSplit", defocusWaves: 0.18, astigmatismWaves: 0.18 }
  ];
  const results = Object.fromEntries(definitions.map((definition) => [
    definition.key,
    calculateLensComplexPupilMtf({
      ...buildCanonicalAberrationPupil(definition),
      wavelengthNm: 587.6,
      workingFNumber: 4,
      sampleCount: 101,
      includeArrays: false
  ...
  const frequencies = [0.3, 0.5];
  const axes = ["sagittal", "tangential"];
  const checkpoints = definitions.map((definition) => ({
    key: definition.key,
    defocusWaves: definition.defocusWaves,
    astigmatismWaves: definition.astigmatismWaves,
    samples: frequencies.flatMap((normalizedFrequency) => axes.map((axis) => ({
      normalizedFrequency,
      axis,
      value: canonicalMtfValueAt(results[definition.key], axis, normalizedFrequency)
    })))
  }));
  const finiteCheckpointValues = checkpoints.flatMap((item) => item.samples.map((sample) => sample.value));
  const maximumDefocusAxisDifference = Math.max(...frequencies.map((frequency) => Math.abs(
    canonicalMtfValueAt(results.positiveDefocus, "sagittal", frequency)
      - canonicalMtfValueAt(results.positiveDefocus, "tangential", frequency)
  )));
  const maximumDefocusConjugateDifference = Math.max(...frequencies.flatMap((frequency) => axes.map((axis) => Math.abs(
    canonicalMtfValueAt(results.positiveDefocus, axis, frequency)
      - canonicalMtfValueAt(results.negativeDefocus, axis, frequency)
  ))));
  const minimumDefocusMtfLoss = Math.min(...frequencies.flatMap((frequency) => axes.map((axis) => (
    canonicalMtfValueAt(results.ideal, axis, frequency)
      - canonicalMtfValueAt(results.positiveDefocus, axis, frequency)
  ))));
  const astigmatismSeparationAt30 = Math.abs(
    canonicalMtfValueAt(results.astigmaticFocusSplit, "sagittal", 0.3)
      - canonicalMtfValueAt(results.astigmaticFocusSplit, "tangential", 0.3)
  );
  const tolerances = {
    maximumDefocusAxisDifference: 0.002,
    maximumDefocusConjugateDifference: 0.000001,
    minimumDefocusMtfLoss: 0.1,
    minimumAstigmatismSeparation: 0.2
  };
  ...
  const passed = zeroAndRangePassed
    && finiteCheckpointValues.length === definitions.length * frequencies.length * axes.length
    && finiteCheckpointValues.every((value) => Number.isFinite(value) && value >= 0 && value <= 1)
    && maximumDefocusAxisDifference <= tolerances.maximumDefocusAxisDifference
    && maximumDefocusConjugateDifference <= tolerances.maximumDefocusConjugateDifference
    && minimumDefocusMtfLoss >= tolerances.minimumDefocusMtfLoss
    && astigmatismSeparationAt30 >= tolerances.minimumAstigmatismSeparation;
  canonicalAberrationValidationCache = Object.freeze({
    status: passed ? "passed" : "failed",
    method: "deterministic complex-pupil defocus conjugacy and astigmatic S/T focus-split validation",
    gridSize: 128,
    wavelengthNm: 587.6,
    workingFNumber: 4,
    checkpoints,
    ...
    diagnostics: {
      zeroAndRangePassed,
      maximumDefocusAxisDifference,
      maximumDefocusConjugateDifference,
      minimumDefocusMtfLoss,
      astigmatismSeparationAt30
    }
  });
